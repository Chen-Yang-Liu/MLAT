...
class CNN_Encoder(nn.Module):
    ...
    def __init__(self, NetType, encoded_image_size=14, attention_method="ByPixel"):
        ...
        net = torchvision.models.inception_v3(pretrained=True, transform_input=False) if NetType == 'inception_v3' else \
              ...
              torchvision.models.vgg19(pretrained=True) if NetType == 'vgg19' else \
              torchvision.models.resnet50(pretrained=True) if NetType == 'resnet50' else \
              torchvision.models.resnet101(pretrained=True) if NetType == 'resnet101' else torchvision.models.resnet152(pretrained=True)
        ...
        modules = list(net.children())[:-1] if NetType == 'inception_v3' or NetType == 'vgg16' or NetType == 'vgg19' else list(net.children())[:-2]
        ...
        #     self.conv3 = nn.Conv2d(in_channels=1024, out_channels=1024, kernel_size=1, stride=1)
        #     self.conv2 = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=1, stride=1)
            # self.conv1 = nn.Conv2d(in_channels=256, out_channels=2048, kernel_size=1, stride=1)
            # self.conv4_3_2_1 = nn.Conv2d(in_channels=1024, out_channels=2048, kernel_size=1, stride=1)
    ...
    def forward(self, images):
        ...
        """
        # from basefull;
        # module_net1 = list(self.net.children())[:8]
        # # module_net.append(list(self.net.children())[-1])
        # net = nn.Sequential(*module_net1)
        # out = net(images)
        ...
        # fusion4 = self.conv4(out4)  #
        # fusion3 = self.conv3(out3)  #
        # fusion2 = self.conv2(out2)
        # out = torch.cat([F.interpolate(fusion4, scale_factor=2), fusion3], dim=1)
        # fusion4_3 = self.conv3(out3) + F.interpolate(fusion4, scale_factor=2) #1024
        # fusion4_3_2 = self.conv2(out2) + F.interpolate(fusion4_3, scale_factor=2)
        # fusion4_3_2_1 = self.conv1(out1) + F.interpolate(fusion4_3_2, scale_factor=2)
        # out = self.conv4_3_2_1(fusion4_3)  # 融合层数
        # out = fusion4_3  # 融合层数
        # out = fusion4_3_2
        # out = fusion4_3_2_1
