...
def evaluate_lstm(args):
    ...
    loader = torch.utils.data.DataLoader(
        ...
        batch_size=1, shuffle=True, num_workers=0, pin_memory=True) #FIXME: workers was 1, but there will be an error.
    ...
    with torch.no_grad():
        for i, (image, caps, caplens, allcaps) in enumerate(
            ...
            while True:
                ...
                # FIXME：每一时刻均输入feature
                # h, c = decoder.decode_step(torch.cat([embeddings, encoder_out.mean(dim=1)], dim=1), (h, c))  # (s, decoder_dim)

                # FIXME：仅初始时刻输入feature
...
def evaluate_lstm_attention(args):
    """
    Evaluation for decoder_mode: lstm
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', default="./models_checkpoint_GRSL/TR2/", help='model checkpoint.')
    ...
    for encoder_layers, decoder_layers in [(3, 3)]:  # ,,(0,6),(2,2),
        ...
        for i in range(len(filename)):
            ...
            if args.decoder_mode == "lstm":
                ...
                metrics = evaluate_lstm_attention(args)
